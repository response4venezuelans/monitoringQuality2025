...
# Returns 1 if population type sum doesn't match new beneficiaries (Direct Assistance only)
is_valid_population_type_disaggregation <- function(indicator_type, population_type_sum, new_beneficiaries_of_month) {
  if_else(
    indicator_type == "Direct Assistance" & population_type_sum != new_beneficiaries_of_month,
    1L, 0L
  )
}

# Returns 1 if AGD sum doesn't match new beneficiaries (Direct Assistance only)
is_valid_age_gender_disaggregation <- function(indicator_type, agd_sum, new_beneficiaries_of_month) {
  if_else(
    indicator_type == "Direct Assistance" & agd_sum != new_beneficiaries_of_month,
    1L, 0L
  )
}
